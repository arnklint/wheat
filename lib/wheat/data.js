...
var Data = module.exports = {
  ...
  article: Git.safe(function article(version, name, callback) {
    ...
    Step(
      function getArticleMarkdown() {
        Git.readFile(version, "articles/" + name + ".md", this);
      },
      ...
      function finish(err, author) {
        ...
        if(props.tags != undefined){
          props.tags = props.tags.split(',').map(function(element){ 
            return element.trim();
          });
        }
  ...
  fullArticle: Git.safe(function fullArticle(version, name, callback) {
    ...
    Step(
      ...
      function loadExtras(err, props) {
        ...
        Git.log("articles/" + name + ".md", this.parallel());
  ...
  author: Git.safe(function author(version, name, callback) {
    Step(
      function getAuthorMarkdown() {
        ...
        Git.readFile(version, "authors/" + name + ".md", this);
      },
  ...
  tags: Git.safe(function articles(version, callback) {
    Step(
      ...
      function processTags(err, articles) {
        if (err) { callback(err); return; }
        var tags = articles.reduce(function (start, element) {
          if (element && element.tags) {
            element.tags.forEach(function(tag) {
              if(start.indexOf(tag) == -1) {
